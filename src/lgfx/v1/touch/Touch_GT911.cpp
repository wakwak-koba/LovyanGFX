...
//----------------------------------------------------------------------------
  ...
  bool Touch_GT911::init(void)
  {
    ...
    _inited = lgfx::i2c::writeBytes(_cfg.i2c_port, _cfg.i2c_addr, gt911cmd_getdata, 3);

    if (_inited)
    {
      std::uint8_t buf[] = { 0x80, 0x56 };
      lgfx::i2c::writeReadBytes(_cfg.i2c_port, _cfg.i2c_addr, buf, 2, buf, 1);
...
//*/
  ...
  void Touch_GT911::sleep(void)
  {
    ...
    lgfx::i2c::writeBytes(_cfg.i2c_port, _cfg.i2c_addr, writedata, 3);
  }
  ...
  std::uint_fast8_t Touch_GT911::getTouchRaw(touch_point_t *tp, std::uint_fast8_t number)
  {
    ...
    {
      std::uint8_t buf;
      lgfx::i2c::writeReadBytes(_cfg.i2c_port, _cfg.i2c_addr, gt911cmd_getdata, 2, &buf, 1);
      if (buf & 0x80)
      {
        _lasttime = nowtime;
        std::int32_t points = std::min(5, buf & 0x0F);
        lgfx::i2c::writeReadBytes(_cfg.i2c_port, _cfg.i2c_addr, gt911cmd_getdata, 2, _readdata, 2 + points * 8);
        lgfx::i2c::writeBytes(_cfg.i2c_port, _cfg.i2c_addr, gt911cmd_getdata, 3);
      }
    }
  ...
  void Touch_GT911::setTouchNums(std::int_fast8_t nums)
  {
    ...
    lgfx::i2c::writeReadBytes(_cfg.i2c_port, _cfg.i2c_addr, buf, 2, &buf[2], 1);
    if (buf[2] != nums)
    {
      buf[2] = nums;
      lgfx::i2c::writeBytes(_cfg.i2c_port, _cfg.i2c_addr, buf, 3);
  ...
  void Touch_GT911::freshConfig(void)
  {
    ...
    lgfx::i2c::writeReadBytes(_cfg.i2c_port, _cfg.i2c_addr, writedata, 2, &writedata[2], 184);

    writedata[0xBA] = calcChecksum(&writedata[2], 184); // 0x80FF checksum
    writedata[0xBB] = 0x01;                             // 0x8100 config fresh
    lgfx::i2c::writeBytes(_cfg.i2c_port, _cfg.i2c_addr, writedata, 188);
  }
