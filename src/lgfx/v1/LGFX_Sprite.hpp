...
//----------------------------------------------------------------------------
  ...
  struct Panel_Sprite : public Panel_NULL
  {
    friend LGFX_Sprite;
...
//----------------------------------------------------------------------------
  ...
  protected:
    ...
    void push_sprite(LovyanGFX* dst, std::int32_t x, std::int32_t y, std::uint32_t transp = ~0u)
    {
...
// return;
      ...
      if (bits >= 8) {
        if (w == 1) {
          ...
          std::uint8_t* dst = &_img8[(x + y * bw) * bytes];
          std::uint8_t c = _color.raw0;
    ...
    void writePixelsDMA_impl(const void* data, std::int32_t length) override
    {
      ...
      do {
        ...
        memcpy(&_img8[(_xptr * _write_conv.bits >> 3) + _yptr * k], src, len);
        src += len;
    ...
    void writePixels_impl(std::int32_t length, pixelcopy_t* param) override
    {
      ...
      do {
        linelength = std::min<std::int32_t>(_xe - _xptr + 1, length);
        param->fp_copy(&_img8[_yptr * k], _xptr, _xptr + linelength, param);
        ptr_advance(linelength);
      } while (length -= linelength);
    }
